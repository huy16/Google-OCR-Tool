...
class GeocoderService extends EventEmitter {
    ...
    async run(inputFilePath, outputDir, jobId, options = {}) {
        try {
            ...
            this.log(`🚀 Bắt đầu Job ${jobId}`);
            // ... (rest of logging) ...
            this.log(`⚙️ Filter: Dự án="${projectCodeFilter}", Tỉnh="${provinceFilter}", Quận="${districtFilter}", Survey="${surveyInfoFilter}"`);
            this.log(`📂 Input: ${path.basename(inputFilePath)}`);
            ...
            this.log(`✅ Cột dữ liệu: Address=${addressCol}, Coords=${coordsCol}, Link=${linkCol}`);

            // 4. Launch Browser
            this.log(`🌍 Đang khởi động Google Chrome... (Headless: ${headless})`);
            ...
            this.log('🌍 Đang truy cập Google Maps...');
            ...
            try {
                ...
                this.log('⚠️ Warning: Map load slow or selector changed.');
            }
            ...
            this.log(`📊 Tổng số dòng cần xử lý: ${totalRowsToProcess}`);
            ...
            for (let i = START_ROW; i <= sheet.rowCount; i++) {
                ...
                this.log(`🔍 [Row ${i}] Tìm: ${searchQuery}`);
                ...
                try {
                    ...
                    try {
                        ...
                        this.log(`⚠️ Input Error: ${errInput.message}`);
                    }
                    ...
                    for (const sel of directShareSelectors) {
                        ...
                        if (btn) {
                            try {
                                await btn.click();
                                foundShare = true;
                                this.log(`   ✨ Found Share button directly: ${sel}`);
                                break;
                            } catch (e) { }
                        }
                    }
                    ...
                    if (!foundShare) {
                        ...
                        if (allResults.length > 0) {
                            this.log(`   Found ${allResults.length} results. Matching...`);
                            ...
                            if (bestMatch.element) {
                                ...
                                for (const sel of directShareSelectors) {
                                    try {
                                        ...
                                        foundShare = true;
                                        this.log(`   ✨ Found Share button after click: ${sel}`);
                                        break;
                                    } catch (e) { }
                                }

                                if (!foundShare) this.log('⚠️ Could not find Share button after clicking result.');
                            }
                        }
                    }
                    ...
                    if (foundShare) {
                        ...
                        try {
                            ...
                            this.log(`⚠️ Share/Copy Error: ${e.message}`);
                        }
                    ...
                    this.log(`❌ Error Row ${i}: ${err.message}`);
                }
            } // End Loop
            ...
            this.log('❌ Lỗi nghiêm trọng: ' + fatalError.message);
        }
    }
    ...
    async convertCsvToExcel(csvPath, excelPath) {
        this.log('📦 Đang đóng gói file Excel cuối cùng...');
    ...
    getChromiumExecPath() {
        ...
        if (process.platform === 'linux') {
            console.log('🐧 Linux detected. Using Puppeteer default (bundled) or system path.');
            // Docker usually sets executablePath via PUPPETEER_EXECUTABLE_PATH env or we rely on bundled.
            // Returning undefined lets Puppeteer launch its bundled Chromium.
            return undefined;
        }
